...
const CAT_SHORT: Record<string, string> = { WASTE: "Sampah", INFRA: "Infra", DISTURB: "Disturb", LAND: "Tanah" };

/** Cek apakah error disebabkan session expired / unauthorized */
...
function useAnalytics() {
  return useQuery<AnalyticsData>({
    ...
    staleTime: 30_000,
    // Jangan retry saat session expired — redirect akan otomatis terjadi
...
function fmtWeekDelta(n: number): string { return n >= 0 ? `+${n}% wk` : `${n}% wk`; }
...
const TOOLTIP_STYLE = {
  background: "rgba(20,25,45,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, fontSize: 12,
};
...
function Analytics() {
  ...
  const kpiCards = data ? [
    { label: "total pengajuan", value: data.kpis.totalReports.toLocaleString("id-ID"), delta: fmtWeekDelta(data.kpis.weekDelta), positive: data.kpis.weekDelta >= 0, icon: Activity, color: "var(--primary)" },
    { label: "rata-rata respons", value: fmtAvgResp(data.kpis.avgResponseHours), delta: fmtDeltaResp(data.kpis.avgDeltaMin), positive: data.kpis.avgDeltaMin <= 0, icon: Clock, color: "var(--accent)" },
    { label: "tingkat penyelesaian", value: `${data.kpis.completionRate}%`, delta: data.kpis.completionDelta >= 0 ? `+${data.kpis.completionDelta} pts` : `${data.kpis.completionDelta} pts`, positive: data.kpis.completionDelta >= 0, icon: CheckCircle2, color: "var(--status-resolved)" },
    { label: "wilayah aktif", value: String(data.kpis.activeDistricts), delta: `+${data.kpis.activeDistrictsThisWeek} minggu ini`, positive: true, icon: TrendingUp, color: "var(--status-progress)" },
  ] : [];

  const catData = data?.categoryBreakdown.map((c) => ({
    name: CAT_META[c.category]?.label ?? c.category, value: c.count, color: CAT_META[c.category]?.color ?? "#888",
  })) ?? [];

  const responseByCat = data?.responseByCat.map((r) => ({ cat: CAT_SHORT[r.cat] ?? r.cat, hours: r.hours })) ?? [];
    ...
    <main className="flex-1 px-5 md:px-10 py-8 max-w-[1500px] w-full mx-auto">
      <header className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl md:text-4xl font-bold text-gradient">Data analitik pengaduan masyarakat</h1>
        ...
        <button
          ...
          className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors disabled:opacity-40 shrink-0"
        >
      ...
      {isError && (
        <div className="glass rounded-2xl p-4 mb-6 flex items-center gap-3 text-sm border border-red-500/20 text-red-400">
          ...
          {sessionExpired ? (
            ...
            <>
              ...
              <button onClick={() => refetch()} className="ml-auto text-xs underline underline-offset-2 hover:no-underline">
                Coba lagi
              </button>
            </>
          )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {isLoading ? Array.from({ length: 4 }).map((_, i) => <SkeletonCard key={i} />) : kpiCards.map((k) => (
          <div key={k.label} className="glass rounded-2xl p-5 shadow-soft relative overflow-hidden">
            <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full opacity-15 blur-2xl pointer-events-none" style={{ background: k.color }} />
            ...
            <div className="text-xs mt-1 relative font-medium" style={{ color: k.positive ? k.color : "#E24B4A" }}>{k.delta}</div>
          </div>
        ))}
      </div>
            ...
            {isLoading ? <SkeletonChart height={288} /> : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data?.trend ?? []} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(value, name) => [Number(value ?? 0), name === "new" ? "Laporan Baru" : "Selesai"]} />
        ...
        <div className="glass rounded-2xl p-6 shadow-soft">
          ...
          <div className="h-52">
            {isLoading ? <SkeletonChart height={208} /> : catData.length === 0 ? (
              <div className="h-full flex items-center justify-center text-xs text-muted-foreground">Belum ada data</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(value, _name, props) => [Number(value ?? 0), (props.payload as { name: string }).name]} />
                </PieChart>
              </ResponsiveContainer>
            )}
          </div>
          <div className="space-y-1.5 mt-3">
            {isLoading ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex items-center justify-between animate-pulse">
                <div className="h-2 w-28 bg-white/10 rounded" /><div className="h-2 w-4 bg-white/10 rounded" />
              </div>
            )) : catData.map((d) => (
              <div key={d.name} className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full shrink-0" style={{ background: d.color }} />{d.name}
                </span>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="glass rounded-2xl p-6 shadow-soft">
          ...
          <p className="text-xs text-muted-foreground mb-4">Rata-rata jam dari pengiriman ke penyelesaian pertama</p>
          <div className="h-64">
            {isLoading ? <SkeletonChart height={256} /> : responseByCat.length === 0 ? (
              <div className="h-full flex items-center justify-center text-xs text-muted-foreground">Belum ada laporan yang diselesaikan</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={responseByCat} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(value) => [`${Number(value ?? 0)}h`, "Rata-rata waktu respons"]} />
                  <defs>
                    <linearGradient id="barGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#82C8E5" /><stop offset="100%" stopColor="#0047AB" />
                    </linearGradient>
                  </defs>
        ...
        <div className="glass rounded-2xl p-6 shadow-soft">
          ...
          <p className="text-xs text-muted-foreground mb-5">Diperanking berdasarkan tingkat penyelesaian</p>
          {isLoading ? (
            <div className="space-y-4">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="animate-pulse">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="h-3 bg-white/10 rounded w-1/2" /><div className="h-3 bg-white/10 rounded w-16" />
                  </div>
            ...
            <div className="text-xs text-muted-foreground">Belum ada data wilayah yang cukup (minimal 2 laporan per wilayah).</div>
          ) : (
            <div className="space-y-4">
                ...
                <div key={d.name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 text-sm">
                      <span className="h-6 w-6 rounded-md glass flex items-center justify-center text-[11px] font-bold text-accent shrink-0">{i + 1}</span>
                      <span className="font-medium truncate max-w-[160px]">{d.name}</span>
                    </div>
                    <div className="text-xs text-muted-foreground tabular-nums shrink-0 ml-2">{d.rate}% · {d.count} laporan</div>
                  </div>
                  <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
                    <div className="h-full rounded-full gradient-accent transition-all duration-500" style={{ width: `${d.rate}%` }} />
                  </div>
                </div>
              ))}
            </div>

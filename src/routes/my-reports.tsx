...
// Sesuaikan dengan variant yang diterima StatusBadge Anda
...
function toShortId(cuid: string): string {
  // Ambil 4 char terakhir, buat mirip "RPT-A1B2"
  return `RPT-${cuid.slice(-4).toUpperCase()}`;
}
...
function MyReports() {
  ...
  // ── Fetch dari /api/reports pakai authFetch (sama seperti submit.tsx) ───────
  useEffect(() => {
    let cancelled = false;
    ...
    async function load() {
      ...
      try {
        ...
        if (res.status === 401) {
          if (!cancelled) setError("Sesi habis. Silakan login kembali.");
          return;
        }
        if (!res.ok) {
    ...
    <main className="flex-1 px-5 md:px-10 py-8 max-w-[1400px] w-full mx-auto">
      ...
      <header className="mb-8">
        ...
        <p className="text-muted-foreground mt-2 text-sm">
          Every report you've submitted and its current resolution status.
        </p>
      </header>
        ...
        <div className="divide-y divide-border">
          ...
          {!loading && !error && list.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="grid grid-cols-[64px_1fr_auto] md:grid-cols-[80px_2.5fr_1.2fr_1fr_1.4fr_100px_40px] gap-4 px-5 py-4 hover:bg-white/[0.03] transition-smooth items-center"
              >
                {/* Gambar */}
                <img
                ...
                <div className="min-w-0">
                  ...
                  {/* Mobile: tampilkan badge di sini */}
                ...
                {/* Lokasi: village, city (≈ subdistrict, city di data lama) */}

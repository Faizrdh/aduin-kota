...
export const Route = createFileRoute("/landing")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Platform pengaduan warga kota secara real-time. Lihat, laporkan, dan pantau permasalahan di kotamu.",
      },
    ],
  }),
...
function FadeIn({
  ...
  const dirs = { up: { y: 32, x: 0 }, left: { y: 0, x: -32 }, right: { y: 0, x: 32 }, none: { y: 0, x: 0 } };
  return (
    <motion.div
      ref={ref}
...
/* ═══════════════════════════════════════════════════════
   ▼▼▼  ILUSTRASI — menggunakan asset SVG  ▼▼▼
═══════════════════════════════════════════════════════ */

/** Ilustrasi 1: Buat Laporan — dari asset */
...
/** Ilustrasi 2: Verifikasi Laporan — dari asset */
function IllusVerifikasi() {
  return (
    <img
      src={verifikasiLaporanSvg}
      alt="Verifikasi Laporan"
      className="w-36 h-36 relative z-10 object-contain drop-shadow-lg"
    />
  );
}

/** Ilustrasi 3: Selesai Laporan — dari asset */
function IllusMasalahSelesai() {
  return (
    <img
      src={selesaiLaporanSvg}
      alt="Masalah Selesai"
      className="w-36 h-36 relative z-10 object-contain drop-shadow-lg"
    />
  );
...
const STEPS_DATA = [
  ...
  },
  {
    ...
    Illustration: IllusMasalahSelesai,
    progress: "100%",
    dots: [true, true, true],
    pulseDelay: 1,
  },
...
/* ─────────────────────────────────────────────
   KARTU LANGKAH (dengan ilustrasi)
───────────────────────────────────────────── */
function StepIllusCard({ step, index }: { step: (typeof STEPS_DATA)[number]; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Nomor langkah */}
      <div
        className="absolute top-3.5 right-3.5 h-7 w-7 rounded-full flex items-center justify-center
                   text-[12px] font-black text-white z-10"
      ...
      <section className="relative w-screen h-screen overflow-hidden">
        ...
        >
          <div className="glass-strong rounded-xl px-3 py-2 flex items-center gap-1.5 shadow-elevated border border-white/10 self-end">
            ...
            <span className="text-muted-foreground text-xs">/ {REPORTS.length} laporan</span>
          </div>
            ...
            >
              ...
              <div className="p-5 flex-1 overflow-y-auto">
                ...
                <div className="mt-5 space-y-3 text-sm">
                  <InfoRow icon={<MapPin size={12} />} label="Lokasi" value={`${selected.region.subdistrict}, ${selected.region.city}`} />
                  ...
                  {[
                    ...
                    <button key={tab.key} onClick={() => setFilterStatus(tab.key)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-smooth flex items-center gap-1.5 ${
                        filterStatus === tab.key ? "gradient-primary text-primary-foreground shadow-glow" : "text-muted-foreground hover:text-foreground"
                      }`}
                    >
                      {tab.label}
          ...
          <div className="relative max-w-2xl mx-auto px-6 text-center">
            <FadeIn>
              ...
              <p className="text-muted-foreground text-sm leading-relaxed mb-8 max-w-md mx-auto">
                Bergabunglah dengan warga yang sudah aktif berkontribusi
        ...
        <div className="mt-4 pt-3 border-t border-white/6 space-y-2.5">
          ...
          <div className="mt-1">
            ...
            <div className="h-1.5 rounded-full bg-white/8 overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
                animate={inView ? { width: report.status === "new" ? "8%" : report.status === "progress" ? "55%" : "100%" } : {}}
            ...
            <div className="flex items-center justify-between mt-1.5">
              {["Dilaporkan", "Diproses", "Selesai"].map((step, si) => {
                const done = si === 0 || (si === 1 && report.status !== "new") || (si === 2 && report.status === "resolved");
                return (

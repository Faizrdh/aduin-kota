...
function useMapReports() {
  ...
  // Simpan raw data untuk lookup AI fields saat detail card dibuka
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      // Bangun lookup map: shortId → ApiReport
  ...
  // Fungsi untuk mendapatkan raw ApiReport berdasarkan Report.id (shortId)
...
function MapPage() {
  ...
  const [selectedRaw,    setSelectedRaw]    = useState<ApiReport | null>(null); // ← untuk AI data
  const [showMobileCats, setShowMobileCats] = useState(false);
  const [showHeatmap,    setShowHeatmap]    = useState(false);

  // Saat laporan di-refresh, update selected juga
  useEffect(() => {
    if (!selected) return;
    const updated = reports.find(r => r.id === selected.id || (r.lat === selected.lat && r.lng === selected.lng));
    if (updated) {
      setSelected(updated);
  ...
  // Handler saat marker diklik — set both Report dan raw ApiReport
  function handleSelect(report: Report | null) {
    setSelected(report);
    if (!report) { setSelectedRaw(null); return; }
  ...
  const toggleAllCats = () =>
    setActiveCats(prev => prev.length === Object.keys(CATEGORIES).length ? [] : Object.keys(CATEGORIES) as Category[]);
  ...
  return (
    <main className="flex-1 relative h-[calc(100vh-0px)] md:h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 p-2 md:p-3">
        <MapClient reports={filtered} onSelect={handleSelect} height="100%" showHeatmap={showHeatmap} />
      </div>
      ...
      <AnimatePresence>
        {error && !loading && (
          <motion.div key="error" initial={{ y: -60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -60, opacity: 0 }}
            ...
            <button onClick={refetch} className="text-xs text-accent hover:underline whitespace-nowrap">Coba lagi</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="absolute top-4 left-4 right-4 flex flex-wrap items-center gap-2 pointer-events-none z-[400]">
        <div className="glass-strong rounded-2xl px-3.5 py-2.5 flex items-center gap-2 pointer-events-auto shadow-elevated flex-1 max-w-xs min-w-0">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari laporan, kota…"
            className="bg-transparent outline-none text-sm w-full placeholder:text-muted-foreground min-w-0" />
          {search && <button onClick={() => setSearch("")} className="text-muted-foreground hover:text-foreground shrink-0"><X size={13} /></button>}
        </div>
        ...
        <div className="glass-strong rounded-2xl p-1.5 items-center gap-1 pointer-events-auto shadow-elevated hidden sm:flex flex-wrap">
          <button onClick={() => setActiveStat("all")}
            className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-smooth ${activeStat === "all" ? "gradient-primary text-primary-foreground shadow-glow" : "text-muted-foreground hover:text-foreground"}`}>
            All
          </button>
          {(Object.keys(STATUSES) as Status[]).map(s => (
            <button key={s} onClick={() => setActiveStat(s)}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-smooth ${activeStat === s ? "bg-white/10 text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
              {STATUSES[s].label}
            </button>
          ))}
        </div>
        ...
        <div className="ml-auto flex items-center gap-2 pointer-events-auto">
          <button onClick={() => setShowHeatmap(p => !p)}
            className={`h-8 px-3 rounded-xl glass-strong flex items-center gap-1.5 shadow-elevated text-xs font-medium transition-smooth ${showHeatmap ? "text-orange-300 bg-orange-500/20 border border-orange-500/40" : "text-muted-foreground hover:text-foreground border border-transparent"}`}>
            <Flame size={13} className={showHeatmap ? "text-orange-400" : ""} />
            <span className="hidden sm:inline">Heatmap</span>
          </button>
          {lastFetch && (
            <button onClick={refetch} title={`Terakhir diperbarui: ${lastFetch.toLocaleTimeString("id-ID")}`}
      ...
      <div className="absolute top-[72px] left-4 right-4 flex sm:hidden items-center gap-1.5 overflow-x-auto scrollbar-none z-[400] pointer-events-auto">
        <div className="glass-strong rounded-2xl p-1 flex items-center gap-1 shadow-elevated shrink-0">
          <button onClick={() => setActiveStat("all")}
            className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-smooth whitespace-nowrap ${activeStat === "all" ? "gradient-primary text-primary-foreground" : "text-muted-foreground"}`}>
            Semua
          </button>
          {(Object.keys(STATUSES) as Status[]).map(s => (
            <button key={s} onClick={() => setActiveStat(s)}
              className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-smooth whitespace-nowrap ${activeStat === s ? "bg-white/10 text-foreground" : "text-muted-foreground"}`}>
              {STATUSES[s].label}
            </button>
          ))}
        </div>
        <button onClick={() => setShowHeatmap(p => !p)}
          className={`glass-strong rounded-2xl px-3 py-2 flex items-center gap-1.5 shadow-elevated text-[11px] font-medium shrink-0 ${showHeatmap ? "text-orange-300 bg-orange-500/20 border border-orange-500/40" : ""}`}>
          <Flame size={12} className={showHeatmap ? "text-orange-400" : "text-accent"} /> Heatmap
        </button>
        <button onClick={() => setShowMobileCats(p => !p)}
      ...
      <AnimatePresence>
        {showHeatmap && (
          <motion.div key="heatmap-legend" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
            ...
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Kepadatan Masalah</p>
            <div className="h-2.5 w-32 rounded-full" style={{ background: "linear-gradient(to right, #00ff00, #ffff00, #ff8800, #ff0000)" }} />
            <div className="flex justify-between mt-1 text-[9px] text-muted-foreground"><span>Rendah</span><span>Tinggi</span></div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="absolute top-24 left-4 glass-strong rounded-2xl p-3 z-[400] pointer-events-auto shadow-elevated w-52 hidden md:block">
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <button onClick={toggleAllCats} className="text-[10px] text-muted-foreground hover:text-foreground transition-smooth">
            {activeCats.length === Object.keys(CATEGORIES).length ? "None" : "All"}
          </button>
        </div>
        <div className="space-y-0.5">
          {(Object.keys(CATEGORIES) as Category[]).map(c => {
            const cat = CATEGORIES[c]; const active = activeCats.includes(c);
            const count = reports.filter(r => r.category === c).length;
            return (
              <button key={c} onClick={() => toggleCat(c)}
                className={`w-full flex items-center gap-3 px-2.5 py-2 rounded-lg transition-smooth text-sm ${active ? "bg-white/8" : "opacity-45 hover:opacity-80"}`}>
                <span className="h-2.5 w-2.5 rounded-sm shrink-0" style={{ background: cat.color, boxShadow: active ? `0 0 8px ${cat.color}` : "none" }} />
                <span className="flex-1 text-left text-[12px] leading-snug">{cat.label}</span>
                <span className="text-[10px] text-muted-foreground tabular-nums">{loading ? "…" : count}</span>
              </button>
            );
          })}
        </div>
      ...
      <AnimatePresence>
        {showMobileCats && (
          <motion.div key="mobile-cats" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}
            ...
            <div className="flex items-center justify-between mb-2 px-1">
              <span className="text-[10px] font-semibold uppercase tracking-wider">Kategori</span>
              <button onClick={() => setShowMobileCats(false)}><X size={13} className="text-muted-foreground" /></button>
            </div>
            {(Object.keys(CATEGORIES) as Category[]).map(c => {
              const cat = CATEGORIES[c]; const active = activeCats.includes(c);
              return (
                <button key={c} onClick={() => toggleCat(c)}
                  className={`w-full flex items-center gap-3 px-2.5 py-2 rounded-lg transition-smooth ${active ? "bg-white/8" : "opacity-45"}`}>
                  <span className="h-2.5 w-2.5 rounded-sm shrink-0" style={{ background: cat.color, boxShadow: active ? `0 0 8px ${cat.color}` : "none" }} />
                  <span className="flex-1 text-left text-[12px]">{cat.label}</span>
                  <span className="text-[10px] text-muted-foreground">{reports.filter(r => r.category === c).length}</span>
                </button>
              );
            })}
          </motion.div>
        ...
        {selected && (
          ...
          >
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              <div className="text-[10px] text-muted-foreground tracking-wider uppercase font-mono">{selected.id}</div>
              <h3 className="font-display text-lg font-semibold mt-1 leading-snug">{selected.title}</h3>

              {/* ── AI Badge di detail card ── */}
              ...
              <div className="mt-5 space-y-3 text-sm">
                <Row icon={<MapPin size={13} />} label="Lokasi"
                  value={`${selected.region.subdistrict}, ${selected.region.district}, ${selected.region.city}`} />
                <Row icon={<span className="text-[10px]">📍</span>} label="Koordinat"
            ...
            <div className="p-4 border-t border-border flex gap-2 shrink-0">
              <button className="flex-1 px-3 py-2 rounded-xl glass text-xs font-medium hover:bg-white/10 transition-smooth">Tugaskan</button>
              <button className="flex-1 px-3 py-2 rounded-xl gradient-primary text-primary-foreground text-xs font-semibold hover:scale-[1.02] transition-smooth">Tandai selesai</button>
            </div>
          </motion.aside>
        )}
      </AnimatePresence>
        ...
        {selected && (
          ...
          >
            ...
            <div className="px-4 pb-2 flex items-start justify-between shrink-0">
              ...
              <button onClick={() => handleSelect(null)} className="h-7 w-7 rounded-full glass flex items-center justify-center">
                <X size={13} />
              </button>
            </div>
            <div className="px-4 pb-4 overflow-y-auto flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed line-clamp-3">{selected.description}</p>
              <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                <MapPin size={11} />
                <span className="truncate">{selected.region.subdistrict}, {selected.region.city}</span>
            ...
            <div className="px-4 pb-5 pt-2 flex gap-2 shrink-0 border-t border-border">
              <button className="flex-1 px-3 py-2.5 rounded-xl glass text-xs font-medium hover:bg-white/10 transition-smooth">Tugaskan</button>
              <button className="flex-1 px-3 py-2.5 rounded-xl gradient-primary text-primary-foreground text-xs font-semibold">Tandai selesai</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function Row({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-start gap-2.5">
      <span className="mt-0.5 h-6 w-6 rounded-md bg-white/5 flex items-center justify-center text-muted-foreground shrink-0">{icon}</span>

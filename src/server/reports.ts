/*eslint-disable*/
// src/server/reports.ts — VERSI UPDATED dengan AI Smart Routing
// ─────────────────────────────────────────────────────────────────────────────
// Perubahan dari versi sebelumnya:
//   1. POST / → memanggil classifyReport() setelah simpan ke DB, lalu update
//   2. PATCH /:id/ai-label → endpoint override manual untuk admin
//   3. GET /all → include kolom ai_label, confidence_score, ai_overridden
// ─────────────────────────────────────────────────────────────────────────────
/*eslint-disable*/
...
router.get("/stats", requireAdmin, async (_req, res, next) => {
  try {
    const [total, pending, inReview, inProgress, resolved, rejected] = await Promise.all([
      prisma.report.count(),
    ...
    return res.json({ total, pending, inProgress: inReview + inProgress, resolved, rejected });
  } catch (err) { next(err); }
});
...
router.get("/all", requireAdmin, async (req, res, next) => {
  try {
    ...
    const aiLabel  = getString(req.query.ai_label); // ← BARU: filter by dinas
    ...
    if (aiLabel  && aiLabel  !== "all") where.ai_label = aiLabel; // ← filter dinas
    if (search) {
      where.OR = [
...
router.get("/public", async (req, res, next) => {
  try {
    ...
    const [reports, total] = await Promise.all([
      prisma.report.findMany({
        ...
        select: {
          ...
          lat: true, lng: true, province: true, city: true, district: true,
          village: true, imageUrl: true, createdAt: true,
          // ── AI fields (publik boleh lihat label, tapi bukan skor detail) ──
          ai_label: true, confidence_score: true,
    ...
    return res.json({ data: reports, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } });
  } catch (err) { next(err); }
});
...
router.get("/heatmap", requireAdmin, async (req, res, next) => {
  try {
    const WEIGHT: Record<ReportStatus, number> = {
      [ReportStatus.PENDING]: 1.0, [ReportStatus.IN_REVIEW]: 0.7,
      [ReportStatus.IN_PROGRESS]: 0.7, [ReportStatus.RESOLVED]: 0.2, [ReportStatus.REJECTED]: 0.1,
    };
    ...
    const hasBbox = !isNaN(swLat) && !isNaN(swLng) && !isNaN(neLat) && !isNaN(neLng) && swLat < neLat && swLng < neLng;
    const where  = hasBbox ? { lat: { gte: swLat, lte: neLat }, lng: { gte: swLng, lte: neLng } } : {};
    const points = await prisma.report.findMany({ where, select: { lat: true, lng: true, status: true }, take: 10_000 });
    const heatData = points.map((p) => [p.lat, p.lng, WEIGHT[p.status] ?? 0.1]);
    res.setHeader("Cache-Control", "public, max-age=30, stale-while-revalidate=60");
...
router.get("/heatmap/public", async (req, res, next) => {
  try {
    const WEIGHT: Partial<Record<ReportStatus, number>> = {
      [ReportStatus.PENDING]: 1.0, [ReportStatus.IN_REVIEW]: 0.7,
      [ReportStatus.IN_PROGRESS]: 0.7, [ReportStatus.RESOLVED]: 0.2,
    };
    const where = { status: { not: ReportStatus.REJECTED } };
    const points = await prisma.report.findMany({ where, select: { lat: true, lng: true, status: true }, take: 10_000 });
    const heatData = points.map((p) => [p.lat, p.lng, WEIGHT[p.status] ?? 0.2]);
    res.setHeader("Cache-Control", "public, max-age=30, stale-while-revalidate=60");
    return res.json({ data: heatData });
  } catch (err) { next(err); }
});
...
router.get("/nearby", optionalAuth, async (req, res, next) => {
  try {
    ...
    if (isNaN(lat) || isNaN(lng)) return res.status(400).json({ error: "lat dan lng wajib." });
    if (lat < -90 || lat > 90 || lng < -180 || lng > 180) return res.status(400).json({ error: "lat/lng di luar rentang." });
    ...
    const category    = categoryRaw && validCats.includes(categoryRaw) ? (categoryRaw as ReportCategory) : undefined;
    ...
    const nearby = candidates
      .map((r) => ({ ...r, distanceMeters: Math.round(haversineMeters(lat, lng, r.lat, r.lng)) }))
...
router.post("/", requireAuth, async (req, res, next) => {
  try {
    const { title, description, category, province, city, district, village, address, imageUrl, lat, lng } = req.body;
    ...
    // ── Simpan Laporan (tanpa AI dulu agar user tidak menunggu) ──────────
    const report = await prisma.report.create({
      data: {
        ...
        province: String(province), city: String(city),
        district: String(district), village: String(village),
        address:  address  ? String(address)  : undefined,
        imageUrl: imageUrl ? String(imageUrl) : undefined,
        // ai_label dan confidence_score masih null saat ini
      },
      include: {
        user:   { select: { id: true, name: true, avatar: true } },
        _count: { select: { joins: true } },
    ...
    // ── Respon ke client SEGERA — jangan block dengan AI call ────────────
    res.status(201).json({ data: report });

    // ── Jalankan AI classification secara ASYNC (fire-and-forget) ────────
    // Laporan sudah tersimpan; jika AI gagal, tidak ada yang rugi.
    classifyReport(String(description))
      .then(async (result) => {
        if (!result) return; // AI gagal / timeout → biarkan ai_label tetap null
      ...
      .catch((err) => {
        // Log error tapi jangan crash — laporan sudah tersimpan
...
//
// Endpoint ini memungkinkan admin mengubah label AI secara manual.
// Setiap perubahan dicatat: ai_overridden=true, ai_override_by, ai_override_at.
//
// Body: { ai_label: "PUPR" }  ← harus salah satu dari DINAS_LABELS
//
router.patch("/:id/ai-label", requireAdmin, async (req, res, next) => {
  try {
    ...
    // Validasi: label harus ada dalam daftar dinas yang dikenal
